...
public class HeadlineNewsFragment extends Fragment  {
    ...
    HeadlineNewsViewModel modelView=new HeadlineNewsViewModel();;
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        modelView.datalist.observe(this, new Observer<List<NewsChannelsBean.ChannelList>>() {
            @Override
            public void onChanged(List<NewsChannelsBean.ChannelList> channelLists) {
                adapter.setChannels(channelLists);
            }
        });
        getLifecycle().addObserver(modelView);
//        modelView.getCachedDataAndLoad();
